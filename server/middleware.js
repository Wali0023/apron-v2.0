...
module.exports = (req, res, next) => {
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      const dbPath = path.join(__dirname, "recipes.json");
      const dbData = JSON.parse(fs.readFileSync(dbPath, "utf8"));
      const { recipes, ingredients } = dbData;

      // Generate ID
      const lastId =
        recipes.length > 0 ? Math.max(...recipes.map((p) => p.id)) : 0;
      const newId = lastId + 1;
      req.body.id = newId;

      // Find course Code
      const courseCode = (req.body.course || "GEN").slice(0, 3).toUpperCase();

      // Generate cuisine Code
      const cuisineCode = (req.body.cuisine || "OTH").slice(0, 3).toUpperCase();

      // Generate Title Code
      const titleCode = (req.body.title || "UNK").slice(0, 3).toUpperCase();

      // SKU Pattern: COR-CUI-TIT-ID
      req.body.sku = `${courseCode}-${cuisineCode}-${titleCode}-${newId}`;
    } catch (error) {
      console.error("Error generating SKU:", error);
      // Fallback unique SKU if generation fails
      req.body.sku = `CAT-CUI-UNK-${Date.now()}`;
    }
  }
